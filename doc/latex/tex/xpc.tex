...
\subsection{xpc::errornumbers}
\label{subsec:xpc_namespace_errornumbers}

   This module implements functions to map \texttt{errno} integers
   to the macro names (e.g. \texttt{EAGAIN}).
   This module can be useful to determine, visibly, the error code.
   The \texttt{perror(3)} function shows a complete message, but
   does not help in figuring out the error code.

   The \texttt{errno\_name(errnum)} function returns a reference
   to the simple string for the error number. For example,
   \texttt{errno\_name(1)} function returns a reference "EPERM".

   The \texttt{error\_number(errname)} does the reverse translation.

   We should make a small program the does this!
